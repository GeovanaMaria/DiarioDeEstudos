...
public class Formatacoes {
    ...
    A expresaao "%.2f" informa a quantidade de casas decimais que será impressa em uma determina variável
            (double ou float).O programa irá imprimir no final da execusao;
    ...
    De modo geral para o printl e o println deve usar-se a concatenação (o sinal de +), para conectar a mensagem
    com as variáveis que serão impressas. Lembrando que a línguagen JAVA é "case sensitive" então todas as
    expressões devem ser escritas de maneira assertiva, respeitando sua pontuação e lógica na escrita do programa
    para que não ocorra erros no código;
...
OBS: para os separadores decimais com ponto, é necessário declarar antes do scanner a seguinte expressão
 ...
 Assim, permite que o programa imprima na expressão (double ou float) o ponto, ao invés da vírgula.
 De modo geral para o printl e o println deve usar-se a concatenação (o sinal de +), para conectar a mensagem com
 as variáveis que serão impressas. Lembrando que a línguagen JAVA é "case sensitive" então todas as expressões
 devem ser escritas de maneira assertiva, respeitando sua pontuação e lógica na escrita do programa para que não ocorra erros no código;
